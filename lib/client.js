const {tap, map, switchMap, retryWhen, delayWhen, share, distinctUntilChanged} = require('rxjs/operators');
...
class Client {
    ...
    _prepareColdEvents$() {
        return new BehaviorSubject().pipe(
            ...
            map(event => {
                event.data.identifiers = this._devicesMap[event.data.id].identifiers;
                return event.data;
            }),
            share()
        );
    ...
    _hasNotLoadedDevices(deviceIds) {
        for (const eventId of deviceIds.filter(x => x)) {
            if (!this._devicesMap[eventId]) {
                return true;
            }
        }
